...
public class TubeBuilderRenderer : MonoBehaviour
{
    ...
    {
        int index = Mathf.Clamp((int)(t * 99f), 0, 99);
        return table[index];
    }
    ...
    public void Rebuild()
    {
        ...
        for (int i = 0; i < segments.Length; i++)
        {
            ...
            if (s.generateTube)
            {
                int ringCount = (s.colorCutoffSegment > 0 && s.colorCutoffSegment < seg) ? seg + 1 : seg;
                if (s.useHardEdges) { totalV += (ringCount - 1) * ring * 4; totalT += (ringCount - 1) * ring * 6; }
                else { totalV += ringCount * ring; totalT += (ringCount - 1) * ring * 6; }
            }
        ...
        for (int si = 0; si < segments.Length; si++)
        {
            ...
            if (s.generateTube) {
                if (s.useHardEdges) {
                    for (int i = 0; i < seg - 1; i++) {
                        ...
                        Vector3 N0 = (i == 0) ? startN : CalculateParallelTransport(curveTan[i], ref Nprev, ref Bprev);
                        Vector3 B0 = Vector3.Cross(curveTan[i], N0);
                        Vector3 N1 = CalculateParallelTransport(curveTan[i+1], ref Nprev, ref Bprev);
                        Vector3 B1 = Vector3.Cross(curveTan[i+1], N1);
                        ...
                        for (int j = 0; j < ring; j++) {
                            ...
                            int bIdx = v;
                            AddVertex(curvePos[i] + (N0 * radialCos[j] + B0 * radialSin[j]) * r0 * rs0, new Vector2((float)j/ring * s.uvTiling.x + s.uvOffset.x, tc0 * s.uvTiling.y + s.uvOffset.y), Color.Lerp(s.startColor, s.endColor, tc0), ref v, ref minBounds, ref maxBounds);
                            AddVertex(curvePos[i] + (N0 * radialCos[j1] + B0 * radialSin[j1]) * r0 * rs1, new Vector2((float)(j+1)/ring * s.uvTiling.x + s.uvOffset.x, tc0 * s.uvTiling.y + s.uvOffset.y), Color.Lerp(s.startColor, s.endColor, tc0), ref v, ref minBounds, ref maxBounds);
                            AddVertex(curvePos[i+1] + (N1 * radialCos[j] + B1 * radialSin[j]) * r1 * rs0, new Vector2((float)j/ring * s.uvTiling.x + s.uvOffset.x, tc1 * s.uvTiling.y + s.uvOffset.y), Color.Lerp(s.startColor, s.endColor, tc1), ref v, ref minBounds, ref maxBounds);
                            AddVertex(curvePos[i+1] + (N1 * radialCos[j1] + B1 * radialSin[j1]) * r1 * rs1, new Vector2((float)(j+1)/ring * s.uvTiling.x + s.uvOffset.x, tc1 * s.uvTiling.y + s.uvOffset.y), Color.Lerp(s.startColor, s.endColor, tc1), ref v, ref minBounds, ref maxBounds);
                    ...
                    for (int i = 0; i < seg; i++) {
                        ...
                        for (int j = 0; j < ring; j++) {
                            ...
                            AddVertex(curvePos[i] + (N * radialCos[j] + B * radialSin[j]) * rad * rs, new Vector2((float)j/ring * s.uvTiling.x + s.uvOffset.x, tc * s.uvTiling.y + s.uvOffset.y), Color.Lerp(s.startColor, s.endColor, tc), ref v, ref minBounds, ref maxBounds);
                        }
            ...
            if (s.startCap.type == TubeCapType.Flat) {
                int ci; BuildFlat(curvePos[0], s.startColor, ref v, out ci, ref minBounds, ref maxBounds);
                if (firstRingIdx >= 0) BuildFanRev(ci, firstRingIdx, ring, ref t);
            } else if (s.startCap.type == TubeCapType.Rounded) {
                if (firstRingIdx >= 0) BuildRoundedCap(verts, firstRingIdx, curvePos[0], GetSampledValue(sampledRadius, 0f), ring, s.startCap, -curveTan[0], startN, startB, s.startColor, true, ref v, ref t, ref minBounds, ref maxBounds);
            } else if (s.startCap.type == TubeCapType.FullSphere) {
                BuildSphere(curvePos[0], s.startCap, s.startColor, -curveTan[0], startN, startB, ref v, ref t, ref minBounds, ref maxBounds);
            }

            if (s.endCap.type == TubeCapType.Flat) {
                int ci; BuildFlat(curvePos[seg-1], s.endColor, ref v, out ci, ref minBounds, ref maxBounds);
                if (lastRingIdx >= 0) BuildFan(ci, lastRingIdx, ring, ref t);
            } else if (s.endCap.type == TubeCapType.Rounded) {
                if (lastRingIdx >= 0) BuildRoundedCap(verts, lastRingIdx, curvePos[seg-1], GetSampledValue(sampledRadius, 1f), ring, s.endCap, curveTan[seg-1], Nprev, Bprev, s.endColor, false, ref v, ref t, ref minBounds, ref maxBounds);
            } else if (s.endCap.type == TubeCapType.FullSphere) {
                BuildSphere(curvePos[seg-1], s.endCap, s.endColor, curveTan[seg-1], Nprev, Bprev, ref v, ref t, ref minBounds, ref maxBounds);
            }

            ApplyBoneWeights(vStart, v, s, boneStartIdx);
        }

        FinalizeMesh(v, t, anyBones, allBones, bindPoses, minBounds, maxBounds);
    }

    private void AddVertex(Vector3 pos, Vector2 uv, Color col, ref int v, ref Vector3 min, ref Vector3 max) {
        verts[v] = pos; uvs[v] = uv; colors[v] = col;
    ...
    private void FinalizeMesh(int v, int t, bool anyBones, List<Transform> bones, List<Matrix4x4> bindPoses, Vector3 min, Vector3 max) {
    ...
    void BuildFlat(Vector3 c, Color col, ref int v, out int ci, ref Vector3 min, ref Vector3 max) { ci = v; AddVertex(c, new Vector2(0.5f, 0.5f), col, ref v, ref min, ref max); }
    void BuildPoint(Vector3 c, Vector3 dir, float sc, float rad, Color col, ref int v, out int ti, ref Vector3 min, ref Vector3 max) { ti = v; AddVertex(c + dir.normalized * (rad * sc), new Vector2(0.5f, 1f), col, ref v, ref min, ref max); }
    ...
    void BuildSphere(Vector3 c, CapSettings cp, Color col, Vector3 ax, Vector3 fN, Vector3 fB, ref int v, ref int t, ref Vector3 min, ref Vector3 max) {
        ...
        for (int iy = 0; iy < g; iy++) {
            ...
            for (int ix = 0; ix < g; ix++) {
                ...
                AddVertex(c + (fN * nL.x + ax * nL.y + fB * nL.z) * cp.sphereRadius, new Vector2(tx, ty), col, ref v, ref min, ref max);
            }
        }
    ...
    void BuildRoundedCap(Vector3[] vA, int rS, Vector3 c, float r, int ring, CapSettings cp, Vector3 ax, Vector3 fN, Vector3 fB, Color col, bool isStart, ref int v, ref int t, ref Vector3 min, ref Vector3 max) {
        ...
        for (int i = 1; i < lat; i++) {
            ...
            for (int j = 0; j < ring; j++) {
                ...
                AddVertex(c + (fN * Mathf.Cos(ph) + fB * Mathf.Sin(ph)) * rad + ax * h, new Vector2(j / (float)ring, tL), col, ref v, ref min, ref max);
            }
        }
        ...
        int pI = v; AddVertex(c + ax * (r * cp.scale), new Vector2(0.5f, 1f), col, ref v, ref min, ref max);
    ...
    void EnsureMesh() {
        if (mesh == null) { mesh = new Mesh(); mesh.name = "TubeBatch"; mesh.MarkDynamic(); mesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32; }
