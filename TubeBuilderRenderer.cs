...
public class TubeBuilderRenderer : MonoBehaviour
{
    ...
    {
        if (curvePos == null || curvePos.Length != seg)
        {
    ...
    void PrecomputeRadialTable(int radialCount)
    {
    ...
    float[] PreSampleCurve(AnimationCurve curve)
    {
    ...
    int GetCapVertCount(CapSettings cp, int r)
    {
        if (cp.type == TubeCapType.None) return 0;
        if (cp.type == TubeCapType.Flat || cp.type == TubeCapType.Point) return r + 1;
        if (cp.type == TubeCapType.Rounded) return (Mathf.Max(2, cp.segments) * r + 1 + r);
        if (cp.type == TubeCapType.FullSphere) { int g = Mathf.Max(3, cp.sphereResolution) + 1; return g * g; }
        return 0;
    }

    int GetCapTriCount(CapSettings cp, int r)
    {
        if (cp.type == TubeCapType.None) return 0;
        if (cp.type == TubeCapType.Flat || cp.type == TubeCapType.Point) return r * 3;
        if (cp.type == TubeCapType.Rounded) return Mathf.Max(2, cp.segments) * r * 6 + r * 3;
    ...
    public void Rebuild()
    {
        ...
        Transform globalLastBone = null; bool isFirstSeg = true;
        ...
        for (int si = 0; si < segments.Length; si++)
        {
            ...
            int boneIdx = allBones.Count;
            if (s.useBones) { globalLastBone = SetupSegmentBones(si, ref allBones, ref bindPoses, globalLastBone, globalNprev); }

            Vector3 startN = globalNprev; Vector3 startB = globalBprev;
            int firstRingIdx = -1; int lastRingIdx = -1;
            ...
            if (s.generateTube) {
                if (s.useHardEdges) {
                    for (int i = 0; i < seg - 1; i++) {
                        ...
                        float v_u0 = (s.uvMapping == UVMappingType.WorldSpace) ? accumulatedDist : tc0;
                        float v_u1 = (s.uvMapping == UVMappingType.WorldSpace) ? (accumulatedDist + dist) : tc1;
                        ...
                        float r0 = sampledRadius[Mathf.Clamp((int)(tc0*99),0,99)], r1 = sampledRadius[Mathf.Clamp((int)(tc1*99),0,99)];
                        for (int j = 0; j < ring; j++) {
                            int j1 = (j + 1) % ring; float rs0 = sampledShape[Mathf.Clamp((int)((float)j/ring*99),0,99)], rs1 = sampledShape[Mathf.Clamp((int)((float)j1/ring*99),0,99)];
                            int bIdx = v;
                            AddVertex(curvePos[i] + (N0 * radialCos[j] + B0 * radialSin[j]) * r0 * rs0, new Vector2((float)j/ring * s.uvTiling.x + s.uvOffset.x, v_u0 * s.uvTiling.y + s.uvOffset.y), tc0, Color.Lerp(s.startColor, s.endColor, tc0), T0c, ref v, ref minB, ref maxB);
                            AddVertex(curvePos[i] + (N0 * radialCos[j1] + B0 * radialSin[j1]) * r0 * rs1, new Vector2((float)(j+1)/ring * s.uvTiling.x + s.uvOffset.x, v_u0 * s.uvTiling.y + s.uvOffset.y), tc0, Color.Lerp(s.startColor, s.endColor, tc0), T0c, ref v, ref minB, ref maxB);
                            AddVertex(curvePos[i+1] + (N1 * radialCos[j] + B1 * radialSin[j]) * r1 * rs0, new Vector2((float)j/ring * s.uvTiling.x + s.uvOffset.x, v_u1 * s.uvTiling.y + s.uvOffset.y), tc1, Color.Lerp(s.startColor, s.endColor, tc1), T1c, ref v, ref minB, ref maxB);
                            AddVertex(curvePos[i+1] + (N1 * radialCos[j1] + B1 * radialSin[j1]) * r1 * rs1, new Vector2((float)(j+1)/ring * s.uvTiling.x + s.uvOffset.x, v_u1 * s.uvTiling.y + s.uvOffset.y), tc1, Color.Lerp(s.startColor, s.endColor, tc1), T1c, ref v, ref minB, ref maxB);
                            tris[t++] = bIdx + 0; tris[t++] = bIdx + 2; tris[t++] = bIdx + 1;
                    ...
                    for (int i = 0; i < seg; i++) {
                        ...
                        float rad = sampledRadius[Mathf.Clamp((int)(tc*99),0,99)]; int rS = v;
                        for (int j = 0; j < ring; j++) {
            ...
            if (s.startCap.type != TubeCapType.None) {
                ...
                if (s.startCap.type == TubeCapType.Flat) { int ci; BuildFlat(curvePos[0], s.startColor, 0f, ref v, out ci, ref minB, ref maxB); BuildFanRev(ci, ringS, ring, ref t); }
                else if (s.startCap.type == TubeCapType.Rounded) BuildRoundedCap(verts, ringS, curvePos[0], r0_c, ring, s.startCap, -T0_c, startN, startB, s.startColor, true, ref v, ref t, ref minB, ref maxB);
                else if (s.startCap.type == TubeCapType.FullSphere) BuildSphere(curvePos[0], s.startCap, s.startColor, -T0_c, startN, startB, 0f, ref v, ref t, ref minB, ref maxB);
                else if (s.startCap.type == TubeCapType.Point) { int ti; BuildPoint(curvePos[0], -T0_c, s.startCap.scale, r0_c, s.startColor, 0f, ref v, out ti, ref minB, ref maxB); BuildFanRev(ti, ringS, ring, ref t); }
            }
            if (s.endCap.type != TubeCapType.None) {
                ...
                else if (s.endCap.type == TubeCapType.FullSphere) BuildSphere(curvePos[seg-1], s.endCap, s.endColor, T1_c, globalNprev, globalBprev, 1f, ref v, ref t, ref minB, ref maxB);
    ...
    private void FinalizeMesh(int v, int t, bool any, List<Transform> b, List<Matrix4x4> bp, Vector3 mi, Vector3 ma) {
        Vector3[] fV = new Vector3[v]; Vector2[] fU = new Vector2[v]; Color[] fC = new Color[v]; Vector4[] fTtan = new Vector4[v]; int[] fTri = new int[t];
        Array.Copy(verts, fV, v); Array.Copy(uvs, fU, v); Array.Copy(colors, fC, v); Array.Copy(tangents, fTtan, v); Array.Copy(tris, fTri, t);
        mesh.Clear(); mesh.vertices = fV; mesh.uv = fU; mesh.colors = fC; mesh.tangents = fTtan; mesh.triangles = fTri;
        if (any) {
            BoneWeight[] fW = new BoneWeight[v]; Array.Copy(weights, fW, v);
            int mB = b.Count - 1; for (int i = 0; i < v; i++) { if (fW[i].boneIndex0 > mB) fW[i].boneIndex0 = 0; if (fW[i].boneIndex1 > mB) fW[i].boneIndex1 = 0; }
            if (showWeightsDebug) { for (int i = 0; i < v; i++) fC[i] = new Color(fW[i].weight0, 0, 1f - fW[i].weight0, 1f); mesh.colors = fC; }
            mesh.bindposes = bp.ToArray(); mesh.boneWeights = fW;
            GetComponent<SkinnedMeshRenderer>().bones = b.ToArray(); GetComponent<SkinnedMeshRenderer>().sharedMesh = mesh;
        }
        mesh.RecalculateNormals(); mesh.bounds = new Bounds((mi + ma) * 0.5f, (ma - mi));
    }
    ...
    void BuildSphere(Vector3 c, CapSettings cp, Color col, Vector3 ax, Vector3 fN, Vector3 fB, float nV, ref int v, ref int t, ref Vector3 mi, ref Vector3 ma) {
    ...
    void ApplyBoneWeights(int sV, int eV, TubeSegment s, int bI) {
        if (!s.useBones) { for (int i = sV; i < eV; i++) { weights[i].boneIndex0 = 0; weights[i].weight0 = 1f; weights[i].boneIndex1 = 0; weights[i].weight1 = 0f; } return; }
        for (int i = sV; i < eV; i++) {
    ...
    Transform SetupSegmentBones(int si, ref List<Transform> allB, ref List<Matrix4x4> bp, Transform lastB, Vector3 startN) {
        ...
        while (segments[si].boneInstances.Count > ct) { int last = segments[si].boneInstances.Count - 1; if (segments[si].boneInstances[last]) DestroyImmediate(segments[si].boneInstances[last].gameObject); segments[si].boneInstances.RemoveAt(last); }
        ...
        for (int b = 0; b < ct; b++) {
            ...
            Vector3 tan = GetBezierTangent(segments[si], t); localN = localN - tan * Vector3.Dot(tan, localN); localN.Normalize();
    ...
    void OnDrawGizmosSelected() {
        ...
        for (int i = 0; i < segments.Length; i++) {
            ...
            for (int j = 0; j < segments[i].boneInstances.Count; j++) {
                ...
                Gizmos.DrawWireSphere(curr.position, 0.01f);
                if (curr.parent != null) {
